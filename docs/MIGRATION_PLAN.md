...
## Phase 4h — Remaining breeder/dealer page regressions

**Context:** Phase 4g closed the most visible regressions but introduced or left behind a second
wave of issues identified by comparing the live pre-migration screenshots against the post-4g
Svelte-rendered breeder page.  All items below affect both the Breeder and Dealer pages unless
noted otherwise.

---

### H1 — Fix Wishlist column: revert key from `'Wishlist Pressure'` back to `'Wishlist'`

**Root cause:** G1 changed `key: 'Wishlist'` → `key: 'Wishlist Pressure'` to match the
*deployed* `website/breeder.html` artifact (generated by an older scraper).  The current
Python (`breeder_matrix.py`) outputs `"Wishlist": f"{wishlist_count} {wishlist_pressure}
{wishlist_delta}"` (e.g. `"69 🔥 →"`).  After a fresh `make generate-website` the JSON key
is `'Wishlist'`, not `'Wishlist Pressure'`, so the column is empty and the wishlist range
slider shows `0 – 0` because `computeRange` finds no matching key.

**Files:** `client/src/breeder-page/index.ts`

- Change column key: `{ key: 'Wishlist Pressure', label: 'Wishlist' }` → `{ key: 'Wishlist', label: 'Wishlist' }`
- Change filter config: `wishlistColumn: 'Wishlist Pressure'` → `wishlistColumn: 'Wishlist'`
- The combined display string `"69 🔥 →"` passes through `computeRange` correctly:
  `parseInt('69 🔥 →'.replace(/^[^0-9.]*/, ''), 10)` → `69` (leading digit, no stripping needed).
- Also update the `test_wishlist_pressure_column_not_empty` test in
  `tests/e2e/test_visual_contracts.py`: rename it
  `test_wishlist_column_not_empty` and update the comment to reference key `'Wishlist'`.

**Dealer impact:** `dealer-page/index.ts` already uses `key: 'Wishlist'` — no change needed
there.

**New Vitest test** (none — purely a data-contract fix; the existing E2E test guards it):
- Update the existing `test_wishlist_column_not_empty` regression guard (renamed above).

**Verify:** `make test-client && make test-e2e`

---

### H2 — Fix Price column: align column key with current Python output

**Root cause:** The current Python (`breeder_matrix.py`) writes
`"Price": format_price_cell(...)` → `"£25.00 ↑"`.  The Svelte column config already has
`key: 'Price'` which is correct.  However, the `tmp/local-testing/` CSV was generated by an
older scraper version that used the key `'Price Trend'` (just the arrow `→`), so the column
appears empty when the stale local CSV is used.

The pre-migration UI labelled this column `"Price Trend"` and showed only the direction arrow.
The migration shows the full `"£25.00 ↑"` value — this is an intentional improvement.

**Required action:**

1. Update the column label from `'Price'` to `'Price Trend'` in `breeder-page/index.ts`
   (`{ key: 'Price', label: 'Price Trend' }`) to keep the heading consistent with the
   pre-migration visual while the value now carries additional price information.  Dealer page
   already has the correct behaviour — check its label too.
2. Regenerate the local test CSV so it matches the current Python output:
   ```bash
   make run-scrape   # or manually copy a fresh artifact into tmp/local-testing/
   make generate-website
   ```
3. Add an **E2E regression test** to `tests/e2e/test_visual_contracts.py`:
   ```python
   def test_price_trend_column_not_empty(breeder_page):
       """Price Trend column renders a non-empty value (key 'Price' in JSON)."""
       first_cell = breeder_page.locator('table tbody tr:first-child td').nth(5)
       assert first_cell.text_content().strip() != ''
   ```
...
### H3 — Restructure filter layout: move Stock Pattern + Search into "More Filters" panel

**Root cause:** `SortableTable.svelte` currently renders the Signal filter row, then the Stock
Pattern filter row, then the Search input, and finally the `"▶ More Filters"` toggle button
(which opens only the price/wishlist sliders).  Pre-migration, *only* the Signal row was
always visible; Stock Pattern and Search lived inside the collapsible "More Filters" panel,
with the `"▶ More Filters"` toggle button inline at the **right end of the Signal row**.

**Target layout (matching pre-migration):**

```
[🎯 Signal:] [Show All (118)] [🔥 Hot (top 10)] [🔥 Hot (47)] [⚠️ Watch (28)] [❌ Avoid (43)]  ▶ More Filters
──────────────────────────────────── (collapsed) ──────────────────────────────────────────────
```
...
Changes:
- Move the `{#if filterConfig.stockPatternFilter}` block **inside** the
  `{#if showAdvanced}` section (before the price/wishlist sliders).
- Move the `{#if filterConfig.showSearch !== false}` block **inside** the
  `{#if showAdvanced}` section (before the price/wishlist sliders).
- Move the `"▶ More Filters"` toggle button out of the standalone `controls-row` div and
  into the **Signal filter row** (`signal-filter-row`) as the last item in
  `filter-buttons-container`, always present (regardless of whether
  `priceColumn`/`wishlistColumn` are set) whenever `stockPatternFilter` or `showSearch` is
  configured.
- Remove the now-empty `controls-row` wrapper div.

**New E2E tests** (`tests/e2e/test_breeder_page_interactions.py` or
`test_visual_contracts.py`):
- On page load: Stock Pattern filter group is **not visible**.
- On page load: Search input is **not visible**.
- After clicking "More Filters": Stock Pattern filter group **is visible**.
- After clicking "More Filters": Search input **is visible**.
- The "More Filters" button is located inside the Signal filter row (assert it shares a
  parent container `.signal-filter-row` with the signal buttons).

**Verify:** `make test-client && make test-e2e`

---

### H4 — Fix "Top 10" button: label and position

**Root cause:** Pre-migration the top-10 shortcut was labelled `"🔥 Hot (top 10)"` and
appeared *before* `"🔥 Hot (n)"` in the Signal row.  Post-4g it is labelled `"Top 10"` and
placed *after* `"❌ Avoid (n)"`.

**File:** `client/src/shared/components/SortableTable.svelte`

- Change the `FilterButton` label from `"Top 10"` to `"🔥 Hot (top 10)"`.
- Move the `{#if filterConfig.signalFilter.top10}` block so the Top-10 button is rendered
  **between** `"Show All"` and `"🔥 Hot (n)"` — i.e. as the second button in the signal
  group.

Example target order:
```
Show All (118) | 🔥 Hot (top 10) | 🔥 Hot (47) | ⚠️ Watch (28) | ❌ Avoid (43)
```

**New Vitest test** (`SortableTable.test.ts`):
- When `signalFilter.top10 = true`, the `"🔥 Hot (top 10)"` button appears before
  `"🔥 Hot (n)"` in the DOM.

**Update existing E2E test:** any assertion on the `"Top 10"` button text should be updated
to `"🔥 Hot (top 10)"`.
...
### H5 — Add row counts to Stock Pattern buttons

**Root cause:** Pre-migration the Stock Pattern buttons showed counts in parentheses
(`"Sustained (48)"`, `"Emerging (10)"`, `"Cyclical (6)"`, `"Always (54)"`).  Post-migration
they show plain labels with no counts.

**File:** `client/src/shared/components/SortableTable.svelte`

Add a `stockPatternCounts` derived value using the same pattern as `signalCounts`:
...
Replace the static `STOCK_PATTERN_BUTTONS` constant with a `$derived` array that injects
the count into each label:
...
**New Vitest test** (`SortableTable.test.ts`):
- Render with 3 Sustained rows and 2 Emerging rows; assert `"Sustained (3)"`, `"Emerging (2)"`,
  `"Show All (5)"` button labels are present.

**Update existing E2E test:** the "Show All" stock pattern button text now includes a count —
update any assertion that matches the exact string `"Show All"` to use a `re.compile` or
`has_text` with partial match.

**Verify:** `make test-client && make test-e2e`

---

### H6 — Add emoji labels to filter rows

**Root cause:** Pre-migration the Signal, Stock Pattern, and Search filter rows had distinctive
emoji prefix labels (`"🎯 Signal:"`, `"📊 Stock Pattern:"`, `"🔍 Search:"`).  Post-migration
the labels are plain text (`"Signal:"`, `"Stock Pattern:"`) or absent (Search uses only a
placeholder).

**File:** `client/src/shared/components/SortableTable.svelte`

- Signal row label: `"🎯 Signal:"`
- Stock Pattern row label: `"📊 Stock Pattern:"` (inside the `showAdvanced` panel, per H3)
- Search: render a label `"🔍 Search:"` above or inline with the `<SearchInput>` (inside
  the `showAdvanced` panel, per H3)

> These are cosmetic string changes; no logic change needed.

**New E2E test** (`tests/e2e/test_visual_contracts.py`):
- Assert `"🎯"` is visible in the Signal filter row on page load.
- After expanding More Filters, assert `"📊"` and `"🔍"` are visible.
