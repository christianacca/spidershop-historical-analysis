...
**Activate-time eviction (stale species HTML fix):** During SW activation, all non-main-page entries are deleted from the `html-pages` cache. This is necessary because species detail pages are only lazily cached (on first visit, not pre-fetched), so after a deploy they could contain outdated HTML. HTML pages contain inline scripts (including the view-transition `pagereveal` handler and the `pagehide` scroll-save listener). If a stale species page HTML is served after a deploy, those inline scripts are the old version and features like view transitions and scroll restoration break silently. Evicting non-main species pages on activate means the first post-update navigation to any species page fetches fresh HTML from the network — one small latency hit, but always correct.
