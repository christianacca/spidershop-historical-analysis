...
from sparkline_helpers import generate_sparkline, extract_historical_values, generate_stock_availability_sparkline
...
def build_dealer_supply_risk_table(history_rows):
    ...
    for (sci, size), present_runs in present_runs_map.items():
        ...
        price_history_values = extract_historical_values((sci, size), by_run, runs, "price_gbp", max_runs=8)
        price_history_sparkline = generate_sparkline(price_history_values, max_length=8)
        
        wishlist_history_values = extract_historical_values((sci, size), by_run, runs, "wishlist_count", max_runs=8)
